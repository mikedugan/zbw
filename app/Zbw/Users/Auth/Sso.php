...
class Sso
{
    ...
    public function __construct() {
        ...
        $this->base = \Config::get('zbw.sso.base');

        // Store consumer credentials
        $this->consumer = new OAuthConsumer(\Config::get('zbw.sso.key'), \Config::get('zbw.sso.secret'));

        // if signature method is defined, set the signature method now (can be set or changed later)
        $this->signature(\Config::get('zbw.sso.method'), \Config::get('zbw.sso.cert'));
    }
    ...
    public function signature($signature, $private_key = false)
    {
        ...
        if ($signature == 'RSA' || $signature == 'RSA-SHA1') {
            ...
            $this->signature = new SSO_OAuthSignatureMethod_RSA_SHA1(
              $private_key
            );
            ...
            if ($sso->request->result == 'success') {
                ...
                if ($sso->token->oauth_callback_confirmed == 'true') {
                    ...
                    $this->token = new OAuthConsumer(
                      $sso->token->oauth_token,
                      $sso->token->oauth_token_secret
                    );
    ...
    public function sendToVatsim()
    {
        ...
        header(
          "Location: " . $this->base . $this->loc_login . $this->token->key
        );
        die();
    ...
    private function curlRequest($url, $requestString)
    {
        ...
        curl_setopt_array(
          ...
          array(
            ...
            // do not output the returned data to the user
            CURLOPT_TIMEOUT        => $this->timeout,
            // time out the request after this number of seconds
            CURLOPT_POST           => 1,
            // we are sending this via post
            CURLOPT_POSTFIELDS     => $requestString
            // a query string to be posted (key1=value1&key2=value2)
          )
        );
